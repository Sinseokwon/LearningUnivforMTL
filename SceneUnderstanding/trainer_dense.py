...
# from weight_method import WeightMethods
...
parser.add_argument('--optimizer', default='sgd', type=str, help='weight for IRM')
parser.add_argument('--scheduler', default='cos', type=str, help='weight for IRM')
...
for index in range(total_epoch):
    ...
    for k in range(train_batch):
        ...
        if opt.weight in ['equal', 'dwa']:
            train_pred,rep = model(train_data, return_representation=True)
...
with open('result/mtl_{}_{}_{}_{}_{}_{}_{}_{}_{}test_perform.txt'.format(opt.network, opt.dataset, opt.task, opt.weight, opt.grad_method, opt.seed,opt.epoch,opt.optimizer,opt.scheduler),'w',encoding='UTF-8') as f:
