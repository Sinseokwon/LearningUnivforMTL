...
class MTLDeepLabv3(nn.Module):
    ...
    def forward(self, x,return_representation=False):
        ...
        rep = self.shared_layer4(x)

        # Task specific decoders
        out = [0 for _ in self.tasks]
        for i, t in enumerate(self.tasks):
            out[i] = F.interpolate(self.decoders[i](rep), size=[im_h, im_w], mode='bilinear', align_corners=True)
            if t == 'normal':
                out[i] = out[i] / torch.norm(out[i], p=2, dim=1, keepdim=True)
        if return_representation:
            return out, rep
    ...
    def dummy_parameter(self):
        return (p for n, p in self.named_parameters() if "dummy" in n)
    def get_last_shared(self):
        return self.shared_layer4.parameters()
